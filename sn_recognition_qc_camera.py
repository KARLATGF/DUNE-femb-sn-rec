...
def read_barcode(image, position, barcode_type):
    # TO DO: Modify to also read SN from the SN box.
    x, y, w, h = position
...
def correct_ocr(ocr_result, chip_number, side):
    ...
    # Fixing errors with "ColdADC" for specified chips
    ...
    # Correcting Serial Number impurities: Remove "-" or "." from serial numbers for specific chips
...
def save_reduced_image(image_path, directory_name, suffix, max_dimension=1600):
    ...
    # Save the resized image for HWDB
...
def main_process(image_path_front, image_path_back, batch_results_dir):
    ...
    #directory_name = os.path.join(os.getcwd(), sanitized_name) #results/sanitized_name
    #directory_name = os.path.join(os.getcwd(), "results", sanitized_name)
    directory_name = os.path.join(os.getcwd(), batch_results_dir, sanitized_name)
...
def process_all_batches(base_image_dir, results_dir):

    # Get all batch directories
    batch_dirs = [d for d in os.listdir(base_image_dir) if os.path.isdir(os.path.join(base_image_dir, d)) and d.startswith("batch_")]

    for batch_dir in batch_dirs:
        batch_path = os.path.join(base_image_dir, batch_dir)
        print(f"*********** Processing {batch_dir}")

        # Create a corresponding results subdirectory
        batch_results_dir = os.path.join(results_dir, batch_dir)
        if not os.path.exists(batch_results_dir):
            os.makedirs(batch_results_dir)

        # Collect all image filenames in the batch directory
        image_files = sorted(os.listdir(batch_path))

        # Organize images by their identifier
        image_pairs = {}
        for filename in image_files:
            ...
            identifier, rest = parts
            if identifier.isdigit():  # Ensure the identifier is a number
                if identifier not in image_pairs:
                    image_pairs[identifier] = {"FRONT": None, "BACK": None}

                if "FRONT" in filename:
                    image_pairs[identifier]["FRONT"] = os.path.join(batch_path, filename)
                elif "BACK" in filename:
                    image_pairs[identifier]["BACK"] = os.path.join(batch_path, filename)

        # Process valid image pairs
        for identifier, paths in image_pairs.items():
            front_path = paths["FRONT"]
            back_path = paths["BACK"]

            if front_path and back_path:
                print(f"Processing pair: {os.path.basename(front_path)} and {os.path.basename(back_path)}")
                #main_process(front_path, back_path)
                main_process(front_path, back_path, batch_results_dir)
            else:
                if not back_path:
                    print(f"Missing BACK image for {identifier}_FRONT")
                if not front_path:
                    print(f"Missing FRONT image for {identifier}_BACK")
...
process_all_batches(base_image_dir, results_dir)
